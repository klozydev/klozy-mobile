...
///
/// A reel must tag at least one product, so the "create a reel" row is disabled
/// (greyed, "add a product first" subtitle) until the user has a listing.
class EntrySheetWidget extends StatefulWidget {
  const EntrySheetWidget({super.key});

  @override
  State<EntrySheetWidget> createState() => _EntrySheetWidgetState();
}

class _EntrySheetWidgetState extends State<EntrySheetWidget> {
  late final Future<List<Product>> _products;

  @override
  void initState() {
    super.initState();
    _products = locator<ReelsRepository>().myProducts();
  }
        ...
        FutureBuilder<List<Product>>(
          ...
          builder: (BuildContext context, AsyncSnapshot<List<Product>> snap) {
            ...
            return _EntryRow(
              ...
              sub: hasProducts
                  ? context.l10N.entry_create_reel_sub
                  : context.l10N.entry_create_reel_needs_product,
              enabled: hasProducts,
