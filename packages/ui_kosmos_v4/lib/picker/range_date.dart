...
/// {@category Date}
///
/// Picker de date, permet de récupérer une date via un Objet [dz.Tuple2<DateTime?, DateTime?>].
/// Vous pouvez fournir une plage de date sélectionné par défaut ([initialSelectedDate]),
...
/// les dates sélectionnables.
///
/// Exemple:
/// ![range_picker](../../images/picker/range_picker.png)
///
/// ```dart
/// Button(
///   text: "Range Picker",
...
class RangeDatePicker extends ConsumerStatefulWidget {
  ...
  final dz.Tuple2<DateTime?, DateTime?>? initialSelectedDate;
...
class _RangeDatePickerState extends ConsumerState<RangeDatePicker> {
  ...
  late dz.Tuple2<DateTime?, DateTime?> _selectedDate = dz.Tuple2(
      widget.initialSelectedDate?.value1, widget.initialSelectedDate?.value2);
  final ValueNotifier<int?> _rangeStatusSelector = ValueNotifier(1);

  @override
  Widget build(BuildContext context) {
    final KosmosDatePickerThemeData themeData = loadThemeData(
      widget.theme,
      widget.themeName ?? "date_picker",
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          sh(18),
          _buildRangeSelector(context, themeData),
          sh(12),
          Row(
  ...
  Widget _createHeaderDatePicker(
    ...
    KosmosDatePickerThemeData themeData,
  ) =>
      Row(
        mainAxisSize: MainAxisSize.max,
        children: [
      ...
      tmpDay.add(
        InkWell(
          onTap: () {
            ...
            setState(() {
              if (_rangeStatusSelector.value == 1) {
                if (_selectedDate.value2 != null &&
                    i.isAfter(_selectedDate.value2!)) {
                  // PopUp.showMessage(context, title: "utils.warning".tr(), message: "date.error.first-cant-be-after".tr());
                  return;
                }
                if (i.month == _currentDate.month) {
                  _selectedDate = dz.Tuple2(i, _selectedDate.value2);
                } else {
                  if (i.month < _currentDate.month) {
                ...
                _rangeStatusSelector.value = 2;
              } else if (_rangeStatusSelector.value == 2) {
                if (_selectedDate.value1 != null &&
                    i.isBefore(_selectedDate.value1!)) {
                  // PopUp.showMessage(context, title: "utils.warning".tr(), message: "date.error.last-cant-be-before".tr());
                  return;
                }
                if (i.month == _currentDate.month) {
                  _selectedDate = dz.Tuple2(_selectedDate.value1, i);
                } else {
                  if (i.month < _currentDate.month) {
                    _currentDate = DateTime(_currentDate.year,
                        _currentDate.month - 1, _currentDate.day);
          ...
          child: Container(
            ...
            decoration: _selectedDate.value1 == i || _selectedDate.value2 == i
                ? themeData.activeDateBackgroundDecoration
                : _selectedDate.value1 != null &&
                        _selectedDate.value2 != null &&
                        i.isAfter(_selectedDate.value1!) &&
                        i.isBefore(_selectedDate.value2!)
                    ? themeData.activeRangeDateBackgroundDecoration
                    : themeData.inactiveDateBackgroundDecoration,
            child: Center(
              child: Text(
                i.day.toString(),
                style: (_selectedDate.value1 == i || _selectedDate.value2 == i)
  ...
  Widget _buildRangeSelector(
    ...
    return ValueListenableBuilder(
      valueListenable: _rangeStatusSelector,
        ...
        return Row(
          ...
          children: [
            Expanded(
              child: InkWell(
                onTap: () => val == 1
                    ? _rangeStatusSelector.value = null
                    : _rangeStatusSelector.value = 1,
                child: Container(
                  height: formatHeight(30),
                  decoration: val == 1
                      ? themeData.rangeDataInfoDecoration ??
                          BoxDecoration(
                              borderRadius: BorderRadius.circular(20),
                              border: Border.all(
                                  color: Theme.of(context).primaryColor))
                      : themeData.rangeDataInfoActiveDecoration ??
                          BoxDecoration(
                              borderRadius: BorderRadius.circular(20),
                              border: Border.all(
                  ...
                  child: Center(
                    child: Text(
                      _selectedDate.value1 != null
                          ? "${_selectedDate.value1!.day.toString().padLeft(2, "0")}/${_selectedDate.value1!.month.toString().padLeft(2, "0")}/${_selectedDate.value1!.year}"
                          : "date.range.no_date".tr(),
                      style: val == 1
                          ? themeData.rangeDataInfoActiveTextStyle ??
                              TextStyle(
                                fontSize: sp(12),
                                color: Theme.of(context).primaryColor,
                                fontWeight: FontWeight.w400,
                              )
                          : themeData.rangeDataInfoTextStyle ??
                              TextStyle(
                                fontSize: sp(12),
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Text(
                      _selectedDate.value2 != null
                          ? "${_selectedDate.value2!.day.toString().padLeft(2, "0")}/${_selectedDate.value2!.month.toString().padLeft(2, "0")}/${_selectedDate.value2!.year}"
                          : "date.range.no_date".tr(),
                      style: val == 2
                          ? themeData.rangeDataInfoActiveTextStyle ??
                              TextStyle(
